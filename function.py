...
def upstock(path,dep):
    ...
    # get_diff(data)
    vrich,zort = cleaned_data_for_zort(data,False,dep,str(path).split('.')[0].split('/')[1])
...
# tran_to_svrich('น้อย 15.12.xlsx')
# path = 'slip'
...
def QC(dep,path):
    ...
    old_SKU,idsell = [],[]
    for i in df.index:
        if 'Q' in df.loc[i,'รหัสขาย']:
            old_SKU.append(df.loc[i,'รหัสสต็อก'])
            idsell.append(df.loc[i,'รหัสขาย'])
    old_SKU = tuple(old_SKU)
    # old_SKU = [df.loc[i,'รหัสสินค้า'] for i in df.index]
...
# path = 'stock_20230208175108.xlsx'
# QC('muslin',path)
