...
def simple_upload(request):
    ...
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        path = upstock(f"media/{myfile.name}",dep)
...
def upload_checkstock(request):
    ...
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        fs = FileSystemStorage()
        filename = fs.save(myfile.name, myfile)
        path = check_stock(f"media/{myfile.name}",dep)
...
def upload_checkdiff(request):
    ...
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        fs = FileSystemStorage()
        filename = fs.save(myfile.name, myfile)
        path = get_diff_stock(f"media/{myfile.name}",dep)
        return redirect(f'/{path}')
    return render(request, 'upstock.html')
...
def UpdateOrderVrich(request):
    ...
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        fs = FileSystemStorage()
        filename = fs.save(myfile.name, myfile)
        StartTest(f"media/{myfile.name}",dep)
        messages.success(request,"update เรียบร้อย ... ")
    return render(request, 'index.html')
...
def KorkaiUpload(request):
    ...
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        fs = FileSystemStorage()
        filename = fs.save(myfile.name, myfile)
        path = Korkai(f"media/{myfile.name}")
        return redirect(f'/{path}')
    return render(request, 'index.html')
...
def upstock_page(request):
    ...
    if request.method == 'POST':
        ...
        if request.method == 'POST' and request.FILES['myfile']:
            ...
            fs = FileSystemStorage()
            filename = fs.save(myfile.name, myfile)
            path = fullFill(f"media/{myfile.name}",sec,dep)
            return redirect(f'/{path}')
    return render(request,"upstock.html")
...
def download_zip(sec):
    ...
    with ZipFile(f'media/write_image/{sec}/{sec}.zip', 'w') as zipObj:
        # Iterate over all the files in directory
        for folderName, subfolders, filenames in os.walk(f'media/write_image/{sec}'):
    ...
    for i in os.listdir(f'media/write_image/{sec}'):
        if i.endswith('.png') or i.endswith('.jpg'):
            os.remove(f'media/write_image/{sec}/{i}')
    return sec
...
def upload_image(request):
    ...
    if request.method == 'POST' and request.FILES['myfile']:
        sec = datetime.datetime.now().timestamp()
        sec = str(sec).split('.')[0]
        os.makedirs(f'media/write_image/{sec}', exist_ok=True)
        ...
        for i in myfile:
            ...
            if name:
                write_image_top_right(f"media/write_image/{sec}/{i.name}",name)
            else:
                write_image_top_right(f"media/write_image/{sec}/{i.name}",i.name.split('.')[0].strip())
        if not len(myfile) == 1:
